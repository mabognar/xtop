    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let right_panel = Layout::default()
                .direction(Direction::Vertical)
                .constraints([Constraint::Length(3* edit_integer),Constraint::Fill(1), Constraint::Length((process_info as u16)*8)])
                .split(horizontal[1]);
                ...
                let selected_processes_rows = vec!{
                    ...
                    Row::new(vec![
                        Cell::from("Virt Mem: "), Cell::from(format!("{:.2} MB",
                                                                     process_list[selected_process].virtual_memory() as f64 / 1048576.0)),]),
                    Row::new(vec![
